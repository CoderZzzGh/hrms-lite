...
import { attendanceApi, employeeApi, toList } from '../api'
import { Avatar, Badge, Button, TableWrap, Th, Td, EmptyState, Spinner, StatCard, ProgressBar, TrashIcon } from '../components/ui'
...
export default function Attendance() {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      show('Record deleted', 'success'); load()
  ...
  const sel = { padding: '7px 10px', border: '1px solid var(--border)', borderRadius: 'var(--radius)', fontSize: 13, background: 'var(--surface)', color: 'var(--text)', outline: 'none' }

  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
        <select value={empFilter} onChange={e => setEmpFilter(e.target.value)} style={{ ...sel, minWidth: 180 }}>
        ...
        {(empFilter || dateFilter || statusFilter) && (
          <Button variant="ghost" size="sm" onClick={() => { setEmpFilter(''); setDateFilter(''); setStatusFilter('') }}>Clear filters</Button>
        )}
      </div>
      ...
      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 240, gap: 10, color: 'var(--text3)' }}><Spinner /> Loading records…</div>
      ) : error ? (
        <div style={{ background: 'var(--red-bg)', border: '1px solid var(--red-border)', borderRadius: 'var(--radius)', padding: '14px 18px', color: 'var(--red-text)', fontSize: 13.5 }}>{error}</div>
      ) : records.length === 0 ? (
        ...
        <TableWrap>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              <Th>Employee</Th><Th>ID</Th><Th>Date</Th><Th>Day</Th><Th>Status</Th><Th style={{ textAlign: 'right' }}></Th>
            </tr>
          </thead>
          <tbody>
            {records.map(att => (
              <tr key={att.id} onMouseEnter={e => e.currentTarget.style.background = '#FAFAF8'} onMouseLeave={e => e.currentTarget.style.background = ''}>
                <Td><div style={{ display: 'flex', alignItems: 'center', gap: 9 }}><Avatar name={att.employee_name || '?'} size={28} /><span style={{ fontWeight: 500, fontSize: 13.5 }}>{att.employee_name || '—'}</span></div></Td>
                ...
                <Td style={{ textAlign: 'right' }}><Button variant="ghost" size="sm" onClick={() => handleDelete(att.id)}><TrashIcon /></Button></Td>
              </tr>
            ))}
          </tbody>
        </TableWrap>
      ...
      <MarkAttendanceModal open={modal} onClose={() => setModal(false)} onSave={handleMark} employees={employees} loading={modalLoading} prefillEmployee={empFilter} />
